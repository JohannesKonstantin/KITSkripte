\documentclass[a4paper]{scarticl}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
...
\end{description}
$B := \{n \in \mathbb{N}:$ fuer $m \in \mathbb{N}$ mit $m \leq n \implies n-m \in \mathbb{N}_0$
\begin{description}
...
\item[2)] Gegeben: $m \in \mathbb{N}: C:=\{n \in \mathbb{N} | n + m \in \mathbb{N}\}$\\Zeige C ist induktiv!\\Fuer $m * n $ analog
...
\end{description}
\end{proof}
\subsubsection{Satz 23} $\mathbb{R}$ ist Archimedisch angeordnet $\mathbb{N} \subset \mathbb{R}$ ist \underline{nicht} nach oben beschraenkt\\ insbesondere $\forall a > 0, b \in \mathbb{R} \exists n \in \mathbb{N}: n * a > b$

\begin{proof}
\paragraph*{Angenommen} $\mathbb{N}$ ist nach oben beschraenkt $\underbrace{\implies}_{vollst. Axiom} a = Sup\mathbb{N}\in\mathbb{R}$\\$\implies \alpha - 1$ ist keine obere Schranke fuer $\mathbb{N}$\\$\implies \exists n \in \mathbb{N}, n > \alpha - 1 \iff  \underbrace{n+1}_{\in \mathbb{N}} > \alpha$ (Widerspruch!)
\subparagraph{Waehle}
...
($\mathbb{Z}, +, *$) ist ein kommutativer Ring mit Eins, d.h. alle Koerperaxiome sind erfuellt. Aber es gibt kein inverses Element der Multiplikation. ($\mathbb{Q}, +, *$) ist ein angeordneter Koerper.

\begin{proof}
	Nachrechnen
\end{proof}
\paragraph{Notation}$\mathbb{Z}_p := \{ m\in\mathbb{Z}: m\geq p\}$\\$p \in \mathbb{Z} := p + \mathbb{N}_0$
\paragraph{Alle} $k \mapsto k + p - 1$ bildet $\mathbb{N}$ bijektiv auf $\mathbb{Z}_p$ ab. \\$\Rightarrow$ Alle Eigenschaften von $\mathbb{N}$ gelten auch fuer $\mathbb{Z}_p \forall p \in \mathbb{Z}$\\$\Rightarrow$ Lemma 25: Jede nach unten bzw. oben beschraenkte Teilmenge $\neq \emptyset$ von $\mathbb{Z}$ besitzt ein Minumum bzw. ein Maximum
...
\end{enumerate}
\subsection{Endliche und abzaehlbare Mengen}
\subsubsection{Definition 27 (Cantor)} A, B Mengen heissen gleichmaechtig (oder aequivalent) $A \sim B$, falls es eine Bijektion $f: A \rightarrow B$ gibt.\\B heisst maechtiger als A, $|A| \leq |B|$, falls es eine Injektion $f: A \rightarrow B$ gibt.
\paragraph{Bemerkung}
\begin{enumerate}[1)]
\item $A \sim B$ ist eine Aequivalenzrelation, d.h. reflexiv ($A \sim A$), symmetrisch ($A \sim B \implies B \sim A$) und transitiv ($A \sim B, B \sim C \implies A \sim C$)
\item $A \leq \mathbb{R} \iff \exists$ Surjektion $h: B \rightarrow B$
\item (Cantor) Bernsten-Schroeder-Theorie $|A| \leq |B|$ und $|B| \leq |A| \iff A \sim B$
\end{enumerate}
\subsubsection{Definition 28} Sei $n \in \mathbb{N}_0 [0] := \emptyset$ und rekrusiv $[n + 1] = [n] \cup [n+1]$\\($\implies([n]:=\{k \in \mathbb{N}: 1\leq k \leq n \}$)\\Eine Menge A heisst endlich, falls $\exists n \in \mathbb{N}_0$ mit $A \sim [n]$, sage A hat n Elemente $card(A) := n$ (Kardinalitaet)\\$card\emptyset = 0$ Eine Menge A ist unendlich, falls sie nicht endlich ist.\\\\A heisst abzaehlbar (abzaehlbar unendlich), falls $A \sim \mathbb{N}$\\A ist hoechstens abzaehlbar, falls A endlich ist oder abzaehlbar ist, ansonsten heisst sie ueberabzaehlbar.
\paragraph{Beweis}
\begin{enumerate}[1)]
\item A hoechstens abzaehlbar $\iff \exists$ Surjektion $f: \mathbb{N} \rightarrow A$
...
\item Hilkerts Hotel
\item $[0,1]\sim [0,1)$
\subparagraph{Beweis} Konstruieren $f: [0,1] \rightarrow [0,1)$\\Fuer $x \in [0,1]\backslash (\bigcup_{n\in\mathbb{N}} \{\frac{1}{n}\}):f(x)=x$\\$n\in\mathbb{N}:f(\frac{1}{n}):=\frac{1}{n+1}$ Rechne nach f ist bijektiv!
\end{enumerate}
\subsubsection{Satz 29}
\begin{enumerate}[1)]
\item $A\sim [n], A\sim [m] \implies n = m$ (d.h. Kardinalitaet ist eindeutig)
...
\begin{enumerate}[1)]
\item Ist $A<B,B$ hoechstens abzaehlbar $\implies A$ hoechstens abzaehlbar
\item Jede unendliche Menge besitzt eine abzaehlbare Teilmenge
\item A, B abzaehlbar $\implies A \times B$ abzaehlbar\\ insbesondere $\mathbb{N} \times \mathbb{N}$ abzaehlbar
\item Sei $\{A_k\}$ eine hoechstens abzaehlbare Menge von Menge $A_3, A_2$ hoechstens abzaehlber $\implies$ $\bigcap_k$ $A_k$ ist hoechstens abzaehlbar
\end{enumerate}

\begin{proof}
\begin{enumerate}[1)]
\item O.B.d.A $B = \mathbb{N}$, also $A \subset \mathbb{N}$\\$\implies A$ hat ein kleinstes Element $a_1$\\$\implies A \{a_1\}$ hat ein kleinnstes Element $a_2$\\usw...\\ist $A_n = \emptyset \implies A$ ist endlich, ansonsten $A = \{a_1, a_2, a_3, ...\}$\\Bijektion $f:\mathbb{N}\rightarrow A, n\mapsto a_n \implies A$ ist abzaehlbar
\item ist $A$ unendlich $\implies$ waehle $a_1 \in A$\\
	$a_2 \in A \backslash \{a_1\} =: A_1$
	induktiv $a_{n+1} \in A_n := A_{n+1} \backslash \{a_n\}$\\
	$\implies \{a_1, a_2, ...\}$ abzaehlbar
\item Da $A \sim \mathbb{N}, B \sim \mathbb{N} \implies$ reicht zu zeigen $\mathbb{N} \times \mathbb{N}$ ist abzaehlbar, da $\mathbb{N} \times \mathbb{N}$ unendlich ist $\implies$ zu zeigen $\mathbb{N} \times \mathbb{N}$
\item ist hoechstens abzaehlbar\\
	$\phi (m,n) = 2^m * 3^n$\\
...
\item Schreiben $A_k = \{a_{kn} :\underbrace{1\leq n \leq P_k}_{endlich}, P_k \in \mathbb{N}$ oder $\underbrace{1 \leq n \in \mathbb{N}}_{unendlich}\}$\\
	...
	$$a_{kn} \mapsto (kn) \textbf{ von } A = \bigcup_{k \in I} A_k \rightarrow \mathbb{N} \times \mathbb{N} \leftarrow \textbf{abzaehlbar}$$\\
	sind $A_k, k \in I$ nicht paarweise disjunkt:\\
	$B_1 = A_1, B_2 = A_2 \backslash A_1,$\\
	$B_{n+1} = A_{n+1} \backslash \{A_1 \cup A_2 \cup ... \cup A_n\}$\\
	$\implies B_k$ sind paarweise disjunkt und hoechstens abzaehlbar\\
	$\implies \bigcup_{k} A_k$ ist hoechstens abzaehlbar
\end{enumerate}
\end{proof}
\subsubsection{Korollar 32} $\mathbb{G}$ ist abzaehlbar\\
\begin{proof}
...
\subsubsection{Satz 33} A enthalte mindestens 2 Elemente, $\implies A^\mathbb{N} = \{ f: \mathbb{N} \rightarrow A\}$ ueberabzaehlbar
...
$a < b> \implies I$ ist ueberabzaehlbar
...
\subsubsection{Definition 38} 0! = 1\\
	...
	d.h. $n! = 1 * 2 * 3 ... n)$\\
	$0 \leq k \leq n | \binom{n}{k} := \frac{k!}{k!(n-k)!}$ Binomialkoeffizient
...
\end{proof}
\subsubsection{Binomischer Lehrsatz} $\forall a,b \in \mathbb{R}$ oder $a,b, \in \mathbb{K}$ (Koerper) $\forall n \in \mathbb{N}_0$\\
	...
	$a = 0$ klar, $a \neq 0$,$a+b)^n = a^n(1 + \frac{b}{a})^n$\\
		...
		\begin{enumerate}[a)]
			...
			\item Induktionsannahme fuer n = k gilt:\\
				...
				$$1+\sum_{l=1}^k\right(\underbrace{\binom{k}{l}+\binom{k}{l-1}}_{=\binom{k+1}{l}}\right)x^l + x^{l+1}$$
		\end{enumerate}
	\end{proof}
