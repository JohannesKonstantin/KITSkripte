...
\textbf{Achtung:} $\left\lbrace \emptyset \right\rbrace$ hat ein Element (nämlich die leere Menge)!
...
\begin{itemize}
  ...
  \item Vereinigungsmenge: $M \cup N := \left\lbrace x|x \in M \wedge x \in N\right\rbrace$ \\
  \item Disjunktion: $M$ und $N$ sind disjunkt wenn $M  \cup N = \emptyset$ 
  \item Schnittmenge: $M \cap N := \left\lbrace x|x \in M \vee x \in N\right\rbrace$ 
  \item Differenz: $M \backslash N := \left\lbrace x|x \in M \wedge x \notin N \right\rbrace$ 
  \item Produktmenge: $M \times N:= \left\lbrace (x,y)|x \in M, y \in N \right\rbrace$  \\
  $M_1 \times M_2 \times \ldots \times M_n := \left\lbrace \underbrace{(x_1,x_2,\ldots,x_n)}_{\text{n-Tupel}} : x_j \in M_j, j= 1,\ldots,n \right\rbrace$
\end{itemize}
...
\right\rbrace =\left\lbrace f(x) : x \in A \right\rbrace$
...
\item[4)] Aus $n,m \in \mathbb{N} \text{ und } n < m < n + 1$\\$\implies 0 < \underbrace{m-n}_{\in \mathbb{N}\texttt{ nach 3)}}<1$ (Widerspruch! zu 1))
...
\subsection{Definition 1} Eine reelle Folge ist eine Funktion $f: \mathbb{N} \rightarrow \mathbb{R}, n \mapsto f(n) =: a_n$
