...
\subsubsection{Satz 2: Cauchy-Kriterium für Konvergenz von Reihen}
...
\subsubsection{Korollar 4: Die harmonische Reihe ist divergiert}

\[\sum_{n=1}^\infty \frac{1}{n} \text{ divergiert}\]
...
\subsubsection{Satz 5}

\begin{enumerate}
  \item (Verschiebung des Summantenanfangs)
  
  Sei $(a_n)_{n \geq p}$ eine Folge, $b_j = a_{p+k}, j \in \mathbb{N}_0$
  ...
  \begin{proof}
    ...
    \begin{proof}
      \[\text{Wegen 1) reicht es Reihen der Form } \sum_{n=1}^\infty a_n \text{ zu betrachten} \]
    \end{proof}
  \end{proof}  
  ...
  $g(k)\in \mathbb{N}$ und $a_j=0$, wegen $j \neq g(k) \quad \forall k \in \mathbb{N}$
  
  (d.h. $a_j \neq 0 \Longleftrightarrow \exists k \in \mathbb{N}: j=g(k)$)
  
  Dann haben die beiden Reihen $\sum_{n=1}^\infty a_n$ und $\sum_{k=1}^\infty a_{g(k)}$ dasselbe Konvergenzverhalten.
...
\subsubsection{Satz 8: Leibniz-Konvergenzkriterium}
