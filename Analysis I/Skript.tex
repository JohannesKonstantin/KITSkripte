...
\subsection{Satz 10}
EIne Reine $\sumOI a_n, a_n \geq 0$ konvergiert $\Leftrightarrow \exists k < \infty : \sum_{j=1}^n a_j \leq k: \forall n \in \mathbb{N}$!

\begin{proof}
$S_n \leq S_{n+1}, \forall n$\\
$\implies \sumOI a_n = \sup_{n\in\mathbb{N}} S_n = \sup_{n} \sum_{j=1}^n a_j$\\
Wir setzen $\sup_n S_n = + \infty$, falls die Folge $(S_n)_n$ nicht nach oben beschränkt ist.\\
$\implies \sumOI a_n = \sup_n S_n, a_n \geq 0$
\end{proof}

\subsection{Korollar 11}
Für so eine Reihe $\sumOI a_n, a_n \geq 0$ gilt entweder $\sumOI a_n < \infty$ oder $\sumOI a_n = + \infty$.\\
Erstens bedeutet: sie konvergiert. Letztens bedeutet: sie divergiert bestimmt gegen $\infty$.

\subsection{Satz 12}
Ist $0\leq a_n \leq b_n, \forall n$ und $\sumOI b_n$ konvergiert\\
$\implies \sumOI a_n$ konvergiert und 
$\sumOI a_n \leq \sumOI b_n$
...
$\implies \sumOI a_n \leq \sumOI b_n$
\end{proof}

\subsection{Satz 13 (Cauchy Verdichtungssatz)}
Sei $(a_n)_n$ fallende Nullfolge. Damit $\sumNI a_n$ konvergiert $\Leftrightarrow \sumNI s^n a_{n^n}$ konvergiert.\\
\begin{proof}
...
\begin{enumerate}[1)]
\item $\sumOI \frac{1}{n}$ divergiert\\
	$\sumOI 2^n \frac{1}{n} = \sumOI 1$ divergiert.
\item $\sumOI 2^n \frac{1}{(2^n)^n} = \sumOI	 \underbrace{(2^{1-\gamma})^n}_{<1 \Leftrightarrow \gamma > 1}$ (geometrische Reihe)
\item Ähnlich $\sumOI \frac{1}{n(ln n)}$ konvergiert, wenn $\gamma > 1$. divergiert, wenn $\gamma \leq 1$.
\end{enumerate}
...
\subsection{Satz 15}
Eine absolut konvergente Reihe $\sumOI a_n$ ist konvergent und es gilt:
$$|\sumOI a_n | \leq \sumOI |a_n|$$
\begin{proof}
...
Eine Reihe $\sumOI c_n, c_n > 0$ heißt Majorante, $\sumOI a_n$, falls ein $n_0 \in \mathbb{N}$ existiert.\\
...
\subsection{Satz 18 (Wurzelkriterium)}
Sei $\sumOI a_n$ eine Reihe und $0 < q < 1$ mit $\sqrt[n]{|a_n|} = |a_n|^\frac{1}{n} \leq q$ für fast alle n\\
